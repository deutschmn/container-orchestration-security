...
% TODO Anmerkung Stefan: bei namespaces machts evtl auch sinn zu erklären, auf welchem level die was tun. also zB nur auf API access level, oder setzen die zB auch firewall regeln, das pods in einem namespace nicht mit einem aus einem anderen reden können; machen die irgendwas im filesystem, oä?
...
% TODO write more about service accounts (Anmerkung Stefan) + erklären "whereas for the latter no Kubernetes objects exist." - was hat das für auswirkungen? wie macht man dann noch RBAC damit?
...
Authentication considers no actual permissions. It only determines the identity of the authenticating entity and several attributes about it, such as the username and group memberships. The next step, authorisation, is when the actual mapping step of who is allowed to do what happens. 
...
% TODO Anmerkung Stefan: noch klarer machen warum RBAC das beste für Admission Control is
...
An especially important admission controller with respect to security is the built-in \mycode{PodSecurityPolicy}. It can be used to forbid containers running with privileged users or set containers' file systems to read-only. Both of these are sensible best practices, as further explained in the context of container security in Section \ref{???}. % TODO write this section, then ref to it
% TODO maybe elaborate on PodSecurityPolicies -> Anmerkung Stefan: bei bedarf darf hier gerne ein example PodSecurityPolicy AC rein
...
% TODO Anmerkung Stefan: da würd ich noch ausführen welches networkign eigentlich gemeint is, also das auf der control pane oder das zwischen den nodes oder das von außen, was hat das mit ingress zu tun (ingress eigentlich wo erklärt?), und evtl wie die plugins das ungefähr umsetzen (iptables generieren oä?)
