...
The biggest attack surface on this layer naturally are the applications themselves. Certainly, the whole class of application vulnerabilities cannot be mitigated entirely. Yet, as described before, Kubernetes provides many damage control tools on lower levels that limit an attacker's possibilities, even when an an application has been compromised. 
...
%* No Root
%* seccomp
%    * Docker config overwritten by K8s?
%* SSHD
